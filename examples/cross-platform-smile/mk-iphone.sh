...
mk_all::main() {
    ...
    mk_all::info "Making for iOS device...\n"

    _uname=$(uname)
    if [[ "$_uname" == "Darwin" ]]; then
        _iostoolchain="--toolchain $MYDIR/scripts/iOS-toolchain.cmake"
        _iosminversion="--tocmake -DIOS_MINIMUM_VERSION=15.2"
        ./mk --platform iphone $_iostoolchain $_iosminversion --tocmake -DIOS_TYPE=iphone $@
    else
        mk_all::err "Unexpected Host platform: $_uname\n"
        mk_all::fail "FAILED\n"
